...
DEFINE_bool(skip_pretokenize_for_vllm,
            true,
            "Skip xllm-service pre-tokenization for vLLM backend; "
            "vLLM tokenizes the prompt itself.");

DEFINE_string(internal_api_token,
              "",
              "Shared token for /v1/internal/* endpoints (register, heartbeat). "
              "Empty value disables the check (backwards compatible).");
