...
std::string validate_push(const StateStreamPush& push) {
  if (push.subscriber.empty()) {
    return "State Stream subscriber is empty";
  }
  if (push.timeout_ms == 0 ||
      push.timeout_ms >
          static_cast<uint64_t>(std::numeric_limits<int32_t>::max())) {
    return "State Stream timeout is outside int32 range";
  }
  if (push.batch.contract_version() != provider::kProviderContractVersion ||
      push.batch.master_incarnation().empty() ||
      push.batch.snapshot_seq() == 0 ||
      !xllm::proto::StateBatchKind_IsValid(push.batch.kind()) ||
      push.batch.kind() == xllm::proto::STATE_BATCH_KIND_UNSPECIFIED) {
    return "State Stream batch identity is incomplete";
  }
  return "";
}
...
std::vector<StateStreamPushResult> push_state_stream_batches(
    const std::vector<StateStreamPush>& pushes) {
  std::vector<StateStreamPushResult> results(pushes.size());
  std::vector<std::unique_ptr<PushContext>> contexts;
  contexts.reserve(pushes.size());

  for (size_t index = 0; index < pushes.size(); ++index) {
    const StateStreamPush& push = pushes[index];
    auto context = std::make_unique<PushContext>();
