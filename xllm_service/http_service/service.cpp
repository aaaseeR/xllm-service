...
// the client. The reader takes ownership of redirect_cntl.
template <typename T>
...
// raw client body (no proto re-serialization). Reuses handle_non_stream_response
// (non-stream) and CustomProgressiveReader (stream); the vLLM SSE format passes
// through unchanged.
template <typename T>
void handle_vllm(std::shared_ptr<T> call_data,
  ...
  if (stream) {
    ...
    google::protobuf::Closure* done =
        brpc::NewCallback(&handle_vllm_stream_done<T>, redirect_cntl, call_data);
...
void XllmHttpServiceImpl::Heartbeat(
  ...
  // Reuse the exact same path as the brpc heartbeat (rpc_service): record
  // liveness/incarnation + LoadMetrics + LatencyMetrics. cache_event is empty.
