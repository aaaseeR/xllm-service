...
class Sidecar:
    ...
    def _new_incarnation(self) -> str:
        ...
        return f"{self._args.instance_name}-{token}" if self._args.instance_name \
            else token
    ...
    def _register(self) -> bool:
        ...
        try:
            ...
            meta = build_instance_meta(
                self._args.register_addr, incarnation,
                self._instance_type, self._args.backend_type)
            self._etcd.put(self._key, json.dumps(meta), lease_id)
            self._lease_id = lease_id
            self._incarnation_id = incarnation
            logger.info("registered %s (incarnation=%s, lease=%s, ttl=%ds)",
                        self._key, incarnation, lease_id, self._args.lease_ttl)
            return True
    ...
    def _deregister(self) -> None:
        ...
        try:
            ...
            logger.info("deregistered %s (lease=%s revoked)",
                        self._key, self._lease_id)
        except EtcdError as e:
            # On failure the lease still expires within ttl; not fatal.
            logger.warning("revoke failed (lease expires in <=%ds): %s",
                           self._args.lease_ttl, e)
        finally:
            self._lease_id = None
            self._incarnation_id = None
    ...
    def run(self) -> None:
        ...
        while not self._stop.wait(self._args.keepalive_interval):
            if self._health.is_healthy():
                ...
                fail += 1
                logger.warning("vLLM health probe failed (%d/%d)",
                               fail, self._args.health_fail_threshold)
    ...
    def _wait_until_healthy(self) -> None:
        ...
        while not self._stop.is_set() and not self._health.is_healthy():
            logger.info("waiting for vLLM at %s to become healthy ...",
                        self._args.vllm_url)
    ...
    def _keepalive_or_reregister(self) -> None:
        ...
        try:
            ...
            if ttl <= 0:
                logger.warning("lease %s lost (ttl=0), re-registering",
                               self._lease_id)
                self._lease_id = None
                self._register()
        except EtcdError as e:
    ...
    def _send_heartbeat(self) -> None:
        ...
        try:
            r = requests.post(self._hb_url, json=body, headers=headers,
        ...
        if r.status_code == 409:
            ...
            logger.warning("heartbeat -> HTTP %d: %s",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--register-addr", default=None,
    ...
    p.add_argument("--xllm-service-url", default="http://127.0.0.1:9998",
                   help="master HTTP base URL for /v1/internal/heartbeat")
    p.add_argument("--internal-token",
...
def main(argv: list[str] | None = None) -> int:
    ...
    logger.info("sidecar starting: register %s as backend_type=%s type=%s",
                args.register_addr, args.backend_type, args.instance_type)
    Sidecar(args).run()
    return 0
