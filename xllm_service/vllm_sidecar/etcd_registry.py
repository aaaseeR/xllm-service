...
class EtcdGatewayClient:
    ...
    def __init__(self, endpoints: str | list[str], username: str = "",
                 password: str = "", timeout: float = 3.0) -> None:
    ...
    def _authenticate(self, username: str, password: str) -> None:
        resp = self._post("/v3/auth/authenticate",
    ...
    def _post(self, path: str, body: dict) -> dict:
        ...
        for base in self._bases:
            try:
                r = self._session.post(base + path, json=body,
                                       timeout=self._timeout)
                if r.status_code == 200:
                    return r.json()
                last_err = EtcdError(
                    f"{path} -> HTTP {r.status_code}: {r.text[:200]}")
    ...
    def put(self, key: str, value: str, lease_id: str) -> None:
        self._post("/v3/kv/put",
                   {"key": _b64(key), "value": _b64(value), "lease": lease_id})
