...
def test_attempt_lifecycle_deadline_cancel_fence_and_capacity() -> None:
    ...
    assert not ledger.begin("r0", 0, 1000).accepted

    ledger.activate("inc-1")
    assert ledger.begin("r1", 0, 1000).accepted
    upstream = Closable()
    assert ledger.attach(AttemptKey("r1", 0), upstream)
    assert ledger.query("r1", 0).state == "ATTEMPT_LIFECYCLE_STATE_RUNNING"

    cancelled = ledger.cancel("r1", 0)
    assert cancelled.state == "ATTEMPT_LIFECYCLE_STATE_CANCELLED"
    assert upstream.closed
    before_create = ledger.cancel("r2", 0)
    assert before_create.state == "ATTEMPT_LIFECYCLE_STATE_CANCELLED_BEFORE_CREATE"
    assert not ledger.begin("r2", 0, 1000).accepted
    assert not ledger.begin("r3", 0, 1000).accepted

    clock.now += 5.0
    assert ledger.begin("r3", 0, 1000).accepted
    expiring = Closable()
    assert ledger.attach(AttemptKey("r3", 0), expiring)
    clock.now += 1.0
    assert ledger.reap_expired() == 1
    assert expiring.closed
    assert ledger.query("r3", 0).state == "ATTEMPT_LIFECYCLE_STATE_EXPIRED"
...
def test_only_one_concurrent_submit_wins() -> None:
    ...
    with ThreadPoolExecutor(max_workers=16) as executor:
        results = list(
            executor.map(lambda _: ledger.begin("same", 7, 1000), range(64))
        )
...
def test_new_incarnation_fences_and_closes_old_attempt() -> None:
    ...
    assert upstream.closed
    assert ledger.query("r1", 0).state == "ATTEMPT_LIFECYCLE_STATE_ABSENT"
