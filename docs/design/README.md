...
发生冲突时，版本、代码、测试和完成度门禁以 00 为准，系统边界以 01 为准，V2 基础协议以 02 为准，V2 路由、流控和执行模式分别以 08/09 为准，多 Provider 以 11 为准。03 是建模专项设计，04 是其标注 commit 的 xLLM 能力快照，05 是 V2.5 KV 内存层专项设计，06 记录取舍，07 记录评审进展，10 提供线上证据、问题优先级和后续优化约束，12 是实现事实与远端代码索引，13 是基于权威设计和实现事实生成的汇报视图，外部评估和 90 只提供证据与建议。10/11 的结论需同步为 01/02/03/08/09 的正式接口或门禁后，才构成对应阶段的实现要求；评估结论没有同步到权威设计前，不构成实现要求。
