...
| --- | --- | --- | --- | --- |
| xLLM Native | CPU 公共路径 | 协议、Provider/Event wire、资源状态机、流式语义、deadline、exact 首事件、JSON 解析、异步输出投递 | CPU_VERIFIED | 默认六目标 97/97；output queue 14/14；protocol 14/14；Torch CPU storage/ownership 覆盖 |
...
| DEV-CPU-XLLM | `xllm-dev xllm-test <xllm> native Debug` 默认六目标；定向 output queue 和 protocol 目标 | queue 直接覆盖非连续 view retained storage、源 tensor 释放后所有权及空 tensor | N/A | PASS，97/97 + 14/14 + 14/14；queue 1400 次压力无失败 |
