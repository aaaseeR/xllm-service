...
| xLLM Service + xLLM Native | 模板、Provider/Event Contract、请求身份、execution hold、output reorder、deadline、断连、seq=0 Query 恢复和首输出前 attempt 替换 | 当前外层 xLLM `service_dev` | CPU_VERIFIED | 289/289 tests passed；三个生产二进制 build/link verify |
...
| DEV-CPU-SERVICE | `XLLM_SOURCE_DIR=<xllm> xllm-dev service-test <xllm-service> native Debug` | execution hold/output reorder/deadline/Query recovery/断连/attempt retry 不含 tensor 逻辑 | N/A | PASS，289/289；22 个关键竞态用例各 100 轮无失败 |
| DEV-CPU-VLLM-AGENT | 沙箱内 `python -m pytest -q vllm_sidecar/tests` | N/A，Python 控制面 | 待真实 vLLM-Ascend/NPU | PASS，44/44 |
