...
| --- | --- | --- | --- | --- |
| xLLM Native | CPU 公共路径 | 协议、Provider/Event wire、资源状态机、流式语义、deadline、JSON 解析 | CPU_VERIFIED | 94/94 tests passed |
| xLLM Service + xLLM Native | 模板、Provider/Event Contract、请求身份、execution hold、output reorder、deadline 与 Service 公共路径 | 当前外层 xLLM `service_dev` | CPU_VERIFIED | 179/179 tests passed；三个生产二进制 build/link verify |
...
| DEV-CPU-XLLM | `xllm-dev xllm-test <xllm> native RelWithDebInfo` | 当前目标链接 Torch CPU；新增控制核心不含 tensor 逻辑 | N/A | PASS，94/94 |
...
| DEV-CPU-SERVICE | `XLLM_SOURCE_DIR=<xllm> xllm-dev service-test <xllm-service> native Debug` | 新增 execution hold/output reorder/deadline 不含 tensor 逻辑 | N/A | PASS，179/179 |
...
- 已知缺口/风险：已补齐无设备 `PlatformStream`、CPU Platform 默认值、VMM host 类型
  ...
  当前 `xllm-verify` 不等价于可启动无设备推理服务，不能把 94 项数量当作完整 V2
  证明。
