...
本批验证：xllm-service Debug 三个生产服务目标编译通过，全量 CPU 测试 258/258；
EngineRegistry 10/10；StateStreamOutbox 6/6；StateStreamClient BRPC loopback 4/4；
xLLM Provider 协议 8/8。三个并发关键用例连续 100 轮通过。没有 tensor 数值逻辑，因此
本功能当前无伪造的 Torch CPU 测试项。
...
- 已知缺口/风险：xLLM Native 与 vLLM Agent 尚未在真实 heartbeat 中生成完整
  EngineState，所以当前 ingress 需要调用端补齐后才能形成首个生产 FULL；Link
  PENDING/DEGRADED 周期 reconciler、真实 handshake 状态生成、状态分发指标、
...
- 达到完整 G3 CPU_VERIFIED 仍需完成：xLLM/Agent EngineState 生产、真实 etcd 切主故障
  注入、Link 周期对账、`STATE_BLIND/REGISTRY_BLIND` 迟滞和 readiness 接入。
