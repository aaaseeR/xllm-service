...
| xLLM Native | `REMOTE_PD` | P/D 生产控制路径 | PARTIAL | 已接真实 D KV allocator/scheduler、P dispatch/handoff、四个 V2 RPC、P/D 输出门禁、Service hold、seq 重排、显式请求 deadline、有界首事件 RPC timer 和 seq=0 Query 精确恢复；尚缺统一默认 deadline、Query 失败重试和 CPU loopback 故障矩阵 |
...
- deadline 与 reservation：Service、P、D 只跨 wire 传播每跳重新计算的 remaining
  ...
  复查，Engine 的连续/统一/fixed/zero/OOC 调度边界会停止到期请求并释放 KV。
...
- 明确不支持范围：Service cleanup capacity、deadline capacity 与后台重试参数目前只有 `Options` 保守
  默认值，尚无 profile/CLI 配置、worker RPC/失败 metrics 和稳定 HTTP
  `SERVICE_CLEANUP_CAPACITY_RETRYABLE` 映射；当前 5 分钟 Engine
  reservation 上限已在显式 request deadline 存在时截断，但 30 秒 transfer-start TTL
  尚未从 deadline/profile 推导；未携带字段的请求仍没有统一业务时限；
  ...
  结论。REMOTE_PD output seq/reorder、gap watchdog 和显式 request deadline 已进入
...
| G1/G2 exact 首事件保留与恢复 | xLLM adapter/protocol/4 MiB/field 24；Service Query state、D/P incarnation、attempt、seq、payload 和 index fail-closed；定向恢复/拒绝与 gap 重排各重复 100 轮 | adapter 目标链接 Torch CPU；无 tensor 数值变化 | 待 P/D 故障注入 | PASS（CPU 核心与生产编译）；loopback 待补 |
| 双仓回归 | xLLM 98/98；Service 183/183；xLLM 受影响生产 TU 严格编译；Service 三个生产二进制 build/link verify | 公共测试目标使用 Torch CPU 环境 | N/A | PASS |
...
- 已知缺口/风险：lifecycle callback 仍在状态锁内执行，生产实现必须保持本地、
  ...
  P 首事件有界 timer、seq=0 exact Query 恢复已接线，但 Query 失败后的预算内新
  attempt、默认 deadline 和 fake-brpc 竞态仍属于 G2 欠账；cancel fence/tombstone
  TTL 还须由最大消息寿命证明。
...
- 达到 CPU_VERIFIED 仍需完成：Service 稳定错误映射、cleanup worker metrics 与
  reservation/deadline/profile 配置化和 Gateway 默认 deadline；补齐 CPU
  fake-provider/loopback 端到端 race、seq 重排、deadline 和 fault loop；接入
